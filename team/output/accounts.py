...
class Account:
    ...
    def save_company_profile(
        ...
        start_time: str,
        end_time: str,
        lunch_minutes: Optional[int],
        time_zone: str,
        minimum_wage: Optional[float]
    ) -> Dict[str, Any]:
        """
        Create or update the single company profile record. Only admins may modify it.
        """
        self._ensure_active()
        if not self.is_admin:
            raise PermissionError("Only administrators can update the company profile.")
        ...
        name = _clean_text(company_name, "Company name")
        address = _clean_text(physical_address, "Physical address")
        country_value = _clean_text(country, "Country")
        email_value = _clean_text(email, "Email address").lower()
        if "@" not in email_value or "." not in email_value.split("@")[-1]:
            raise ValueError("Email address appears to be invalid.")
        contact_value = _clean_text(contact_person, "Contact person")
        tz_value = _clean_text(time_zone, "Time zone")

        normalized_days = {
            str(day).strip().title()
            for day in (business_days or [])
            if isinstance(day, str) and day.strip()
        }
        ordered_days = [day for day in BUSINESS_DAY_CHOICES if day in normalized_days]
        if not ordered_days:
            raise ValueError("Select at least one business day.")

        start_time_norm = self._normalize_time(start_time, field_name="business start time")
        end_time_norm = self._normalize_time(end_time, field_name="business end time")
        start_obj = datetime.strptime(start_time_norm, "%H:%M").time()
        end_obj = datetime.strptime(end_time_norm, "%H:%M").time()
        if end_obj <= start_obj:
            raise ValueError("Business end time must be after start time.")

        lunch_value = int(lunch_minutes) if lunch_minutes is not None else 0
        if lunch_value < 0:
            raise ValueError("Lunch minutes cannot be negative.")

        wage_value = float(minimum_wage) if minimum_wage is not None else 0.0
        if wage_value < 0:
            raise ValueError("Minimum wage cannot be negative.")
        ...
        row = self._get_company_profile_row()
        if row is None:
            raise RuntimeError("Company profile could not be saved.")
        return self._format_company_profile(row)
    ...
    def _get_company_profile_row(self) -> Optional[sqlite3.Row]:
        with self._get_connection() as conn:
            cursor = conn.execute(
                """
                SELECT
    ...
    def _format_company_profile(self, row: sqlite3.Row) -> Dict[str, Any]:
        try:
            hours_payload = json.loads(row["business_hours"] or "{}")
        except (TypeError, json.JSONDecodeError):
            hours_payload = {}
        raw_days = hours_payload.get("days") or []
        normalized_days = {
        ...
        ordered_days = [day for day in BUSINESS_DAY_CHOICES if day in normalized_days]
        business_hours = {
            "days": ordered_days or BUSINESS_DAY_CHOICES[:5],
            "start_time": hours_payload.get("start_time", "08:00"),
            "end_time": hours_payload.get("end_time", "17:00")
        }
        return {
            "id": row["id"],
            ...
            "lunch_minutes": int(row["lunch_minutes"]),
            "time_zone": row["time_zone"],
            "minimum_wage": float(row["minimum_wage"]),
